classdef helperclassposegraph < handle
    % A class for doing pose graph optimization
    ...
    methods
        ...
        function obj = helperclassposegraph()
            % Constructor
            obj.node = helperclassnode.empty;   
            obj.edge = helperclassedge.empty;
        end
        
        function insertNode(obj,i_node,pose)
            % adds first node to
            % poseGraph.node -> array of helperclassnode objects
            % TRACKS ROBOTS POSE HISTORY AND ALLOWS FOR GRPAH-BASED SLAM (
            % LOOP CLOSURE + OPTIMIZATION)
            obj.node(i_node) = helperclassnode(i_node,pose);
        end
        
        function insertEdge(obj,i_edge,id_from, id_to, mean, infm)
            obj.edge(i_edge) = helperclassedge(id_from, id_to, mean, infm);
        end
        ...
        function readGraph(obj, vfile, efile)
            ...
            for i_node = 1:size(vertices,2)
                ...
                id = vi(1) + 1;
                pose = vi(2:4);
                obj.node(i_node) = helperclassnode(id, pose);
            end
            fclose(vfid);
        ...
        function linearize(obj)
            ...
            for i_edge = 1:obj.n_edge
                ...
                i_node = ei.id_from;
                j_node = ei.id_to;
                T_z = v2t(ei.mean); % transform matrix
                omega = ei.infm;
