% PCA-based line fitting with deleting & merging
function Line = helperpcalinewithdeletingandmerging(clust) 
% Input
%   scan: points (x, y)
...
for i = 1:number
    ...
    projx = helperprojection(x,y,m(i),n(i));
...
elseif number > 1
    ...
    if abs(a(j)-a(j+1))<alpha && abs(a(j)-ac)<alpha...
         ...
         while true  % projection
             projx = helperprojection([endPoint(:,2*j-1);endPoint(:,2*j+1)],...
