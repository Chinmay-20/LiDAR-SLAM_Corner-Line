function needLoopTrial = helperloopclosurecheck(map)

% First of all, we don't want to detect loop closure too frequently.
%(At least 10 keyscans away from last trial)
% has it been atleast 10 frames since last loop check and robot traveled at
% least 5 meters since last loop closure. 
trialInterval = 1;
for k = length(map.keyscans)-1 : -1 : 1
    if ~map.keyscans(k).loopTried
        trialInterval = trialInterval + 1;
    else
        break;
    end
end
if trialInterval < 10
    needLoopTrial = false;
    return;
end

% Then we don't want to detect loop closure after a newly closed loop
% (At least 5 meters need to be traveled since last closure)
traveledDistance = 0;
for k  = length(map.keyscans)-1 : -1 : 1
    if ~map.keyscans(k).loopClosed
        dT = helperposedifference(map.keyscans(k).pose, map.keyscans(k+1).pose);
        traveledDistance = traveledDistance + norm(dT(1:2));
        if traveledDistance > 5
            break;
        end
    else
        break;
    end
end
if traveledDistance < 5
    needLoopTrial = false;
    return;
end

% Now we see that a loop detection is wanted
needLoopTrial = true;
    