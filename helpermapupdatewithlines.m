...
function restMap = helpermapupdatewithlines(exLine, newLine)
% Input
...
class = helperclusteringmeanshift(templine,radius,alpha);  % Do mean-shift
% class = ones(1,size(templine,2));           % W/o mean-shift
% class = DoCluster2(temppoint,templine,class,dth);
class = helperclusteringhierarchical(temppoint,templine,class,dth);
...
for i = 1 : max(class)      % traverse and merge each class
    ...
    tempMerg = helperpcalinemerging(toMergPts);
    
    if isempty(tempMerg.point)      
